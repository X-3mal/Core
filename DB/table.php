...
abstract class Table {
	...
	static private function _select ( $fields, $where = array() ) {
		...
		$rows = \DBS::main ()->select ( 'SELECT #q FROM `' . \DBS::main ()->escape ( static::$name ) . '` AS `' . static::alias () . '` #q #q ' . ( !empty ( static::$lock[ get_called_class () ] ) ? ' FOR UPDATE' : '' ), $filter->select ( '' ), $filter->join (), $filter->filter ( 'WHERE' ) );
		if ( !empty ( $rows ) ) {
			foreach ( $rows as &$row ) {
				foreach ( $sets as $key ) {
					if ( empty ( $row[ $key ] ) ) {
